...
sub tsv_id_map {
    ...
    while(<$fh>) {
        chomp;
        @F = split /\t/;
        $id_new = $id_mapping->id_new($id_to_space->{id}, 'Feature', $F[0]);

        print $fh_out join("\t", $id_new, @F[1..$#F]), "\n";
    }
...
sub column_names {
    return(qw/id source ac_src id_src type regex description/);
}
