//comments:
//Qianhao 2013-06-25
//this js file is adopted from the pfam website.
//It was modified for the following needs:
// 1. to show the lollipops with different height: 
//		the keyword "score" is added so that the height of lollipops is 
//		calculated by "score * defaultLollipopUnit".
// 2. the definition of the height of lollipops and bridges are separated.
//		see defaultBridgeHeight and so on.
// 3. bug fix: overlapping lollipops show slightly different heights
//------------------------------------------------------------------------------
...
var PfamGraphic = Class.create( {
  ...
  initialize: function( parent, sequence ) {
    ...
    this._imageParams = {
      ...
      defaultBridgeHeight:         20, // default height for bridge
      
      defaultLollipopUnit:         20, // the default hight as unit. The real height should be 
                                       // unit * markup.score
      ...
      motifOpacity:    0.3,      // the opacity of a motif
    ...
    this._options = {
      ...
      tipStyle:  "darkgrey", // style for tooltips
  ...
  _walkSequence: function() {
    ...
    [ s.motifs, s.regions, s.markups ].each( function( j ) {
      $w( 'start end aliStart aliEnd score' ).each( function( i ) {
        i = this._parseInt( i );
      }.bind( this ) );
    }.bind( this ) );
  ...
  _buildMarkups: function() {
    ...
    orderedMarkups.each( function( markup ) {
      ...
      var blnDoubledLolli=false; // the existance of overlapping lollipops
      var checkLollipopStart=start-(1/residueWidth)  //check from there
      var checkLollipopEnd=start+(1/residueWidth)  //chend ends there
      //walk over the range
      for (resinum=checkLollipopStart;resinum<=checkLollipopEnd;resinum++) {
//    	  console.log('resinum:',resinum)
    	  if (h[resinum] !== undefined) {
    		  blnDoubledLolli=true;
    		  break;
    	  }
      }
      
      h[start] = ip.defaultLollipopUnit*markup.score; //ip.defaultMarkupHeight      
      if (blnDoubledLolli) {

        var checkArrayLollipop = h.slice( checkLollipopStart,start);
        checkArrayLollipop.concat(h.slice(start+1,checkLollipopEnd+1))
//        console.log(checkArrayLollipop)
        //if there are already overlapping lollipops with the same height,
        //we increase the height.
        while(checkArrayLollipop.indexOf(h[start])!==-1) {
        	h[start]=h[start]+ip.lollipopToLollipopIncrement;
        }
...
//          start, h[start] );
    ...
    bridgeMarkups.each( function( bridgeMarkup ) {
      ...
      var bridgeHeight = ip.defaultBridgeHeight;
  ...
  _drawBridge: function( bridge ) {
	...
	// the following is the original ones for showing tool-tips of bridges
	// The bridges are choosen to show disulfat bridges, but we want to use
	// the bridges to show frame shifts. So we need to show the start position
	// and a description, see the code block blow.
        // tip.body =
        //   '<div class="tipContent">' +
        //   '  <dl>' +
        //   '    <dt>Coordinates:</dt>' +
        ...
        tip.body =
          '<div class="tipContent">' +
          '  <dl>' +
          '    <dt>Frame shift from:</dt>' +
          '    <dd>' + md.start + '</dd>' +
          '    <dt>Description:</dt>' +
          '    <dd>' + ( md.description || '<span class="na">n/a</span>' ) + '</dd>' +
          '  </dl>' +
          '</div>';
    }
